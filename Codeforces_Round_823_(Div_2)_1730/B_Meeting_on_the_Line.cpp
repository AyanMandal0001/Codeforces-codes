...
bool check(double mid){
 double left=-2e9,right=1e9;
 for(ll i=0;i<n;i++){
    double timeleft=mid-(double)t[i];
    double curl=((double)x[i]-timeleft);
    double curr=((double)x[i]+timeleft);
    left=max(left,curl);
...
void solve() {
 ...
 for(ll i=0;i<n;i++){
    cin>>t[i];
    mx=max(mx,(double)t[i]);
 }
 double lo=mx,hi=1e18;
  double ans=-1;
  ...
  //cout<<ans<<endl;
  cout<<fixed<<setprecision(6)<<xnot<<endl;;
}
